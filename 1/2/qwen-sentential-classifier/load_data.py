...
class DataLoader:
    ...
    def load_csv(self, file_path: str) -> Tuple[List[str], List[int]]:
        ...
        try:
            ...
            print(f"尝试加载CSV文件: {absolute_path}")
            ...
            # 读取CSV文件，不使用列名
            df = pd.read_csv(absolute_path, header=None, names=['label', 'title', 'text'])
            ...
            # 合并标题和文本
            texts = []
            for title, text in tqdm(zip(df['title'], df['text']), total=len(df), desc="合并标题和文本"):
